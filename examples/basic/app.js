...
function main() {
  ...
  const removeRandomPoints = () => {
    store.dispatch(
      mapActions.removeFeatures("points", ["==", "isRandom", true])
    );
  };
        ...
        <div className="mapName">
          <form onSubmit={this.handleSubmit}>
            <div className="mapForm">
              <label className="nameLabel" htmlFor="nameField">
                Change Map Name in Redux store:
              </label>
              <input
                className="nameField"
                placeholder="Enter new map name here"
                type="text"
                id="nameField"
                value={this.state.value}
                onChange={this.updateMapName}
              />
              <button className="sdk-btn" type="submit">
                Update Map Name
              </button>
            </div>
          </form>
          <div className="newName">
            New Redux state.map.name Value: {store.getState().map.name}
          </div>
        </div>
      );
    }
  ...
  const updateMinzoom = () => {
    store.dispatch(
      mapActions.updateLayer("null-island", {
        source: "points",
        type: "circle",
        paint: {
          "circle-radius": 3,
  ...
  ReactDOM.render(
    <Provider store={store}>
      <RendererSwitch setView={handleSetView} onClick={handleClick}>
        <SdkScaleLine />
        <SdkMousePosition
          style={{
            position: "absolute",
  ...
  ReactDOM.render(
    <div>
      <h3>Try it out</h3>
      <button className="sdk-btn" onClick={zoomToNullIsland}>
        Zoom to Null Island
      </button>
      <button className="sdk-btn" onClick={addRandomPoints}>
        Add 10 random points
      </button>
      <button className="sdk-btn blue" onClick={removeRandomPoints}>
        Remove random points
      </button>
      <button className="sdk-btn" onClick={updateMinzoom}>
        Change Minimum Zoom Level on Null Island to 5
      </button>
      <InputField />
